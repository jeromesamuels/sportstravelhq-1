...
class DashboardController extends Controller {
    ...
    public function index(Request $request) {
        $this->data['tc_users'] = User::get()->where("group_id", 4)->count();
        $this->data['ro_users'] = User::get()->where("group_id", 3)->count();
        $this->data['hotels'] = DB::table('hotel_amenities')->get()->count();
        $this->data['rfps'] = Rfp::get()->count();
        $this->data['data_decline'] = Rfp::get()->where("status", '!=', 3)->all();
        $this->data['data_accept'] = Rfp::get()->where("status", 2)->all();
        $this->data['purchases_month'] = Invoices::sum('invoices.amt_paid');
        $this->data['data_hotel'] = Hotel::groupBy('type')->get();
        $this->data['trips'] = usertrips::get()->count();
        $this->data['a_req'] = "101";
