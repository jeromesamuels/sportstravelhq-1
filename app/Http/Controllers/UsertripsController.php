...
class UsertripsController extends Controller {
    ...
    public function __construct() {
        ...
        $this->data = array('pageTitle' => $this->info['title'], 'pageNote' => $this->info['note'], 'pageModule' => 'usertrips', 'return' => self::returnUrl());
    }
    ...
    function show(Request $request, $id) {
            ...
            switch ($task) {
                default:
                    ...
                    if ($validator->passes()) {
                        ...
                        if (!is_null($request->input('apply'))) return redirect($this->module . '/' . $id . '/edit?' . $this->returnUrl())->with('message', __('core.note_success'))->with('status', 'success');
                        return redirect($this->module . '?' . $this->returnUrl())->with('message', __('core.note_success'))->with('status', 'success');
                    } else {
                        return redirect($this->module . '/' . $request->input($this->info['key']) . '/edit')->with('message', __('core.note_error'))->with('status', 'error')->withErrors($validator)->withInput();
                    }
                    break;
                case 'public':
        ...
        public static function display() {
            ...
            if ($mode == 'view') {
                ...
                $params = array('page' => $page, 'limit' => (isset($_GET['rows']) ? filter_var($_GET['rows'], FILTER_VALIDATE_INT) : 100), 'sort' => $info['key'], 'order' => 'ASC', 'params' => '', 'global' => 0);
        ...
        function store_public($request) {
            if (session('gid') != 4) return Redirect::back()->with('message', __('core.note_error'))->with('status', 'error')->withErrors(['message1' => 'Only travel coordinator\'s can book a trip'])->withInput();
            $rules = $this->validateForm();
            $validator = Validator::make($request->all(), $rules);
            if ($validator->passes()) {
                $data = $this->validatePost($request);
        ...
        public function getRFPs($trip_id) {
            ...
            return response()->json(['success' => true, 'view_data' => (string)view('usertrips.public.rfps', $data) ]);
        }
        public function getRFP($rfp_id) {
            $data['rfp'] = Rfp::get()->where("id", $rfp_id)->first();
            return response()->json(['success' => true, 'view_data' => (string)view('usertrips.public.rfp', $data) ]);
        }
        public function compareRFP($rfp_id) {
            $myaar = explode(',', $rfp_id);
            $rfps = Rfp::with('userInfo', 'userInfo.hotel')->whereIn("user_trip_id", $myaar)->get();
            //echo '<pre>';print_r($rfps->toArray());die;
            return response()->json(['success' => true, 'view_data' => (string)view('usertrips.public.comparerfp', compact('rfps')) ]);
        }
        public function acceptRFP($rfp_id) {
            ...
            if ($aggree->user_id != 0) {
                ...
                if ($agree_id === null) {
                    ...
                    return response()->json(['success' => true, 'view_data' => 'Already Accepted !']);
                }
            ...
            return response()->json(['success' => true, 'redirect' => route('hotelmanager.viewAgreements'), 'view_data' => 'Accepted Successfully !', ]);
        }
        public function acceptAgree($rfp_id) {
            ...
            return response()->json(['success' => true, 'view_data' => 'Accepted Successfully !']);
        }
        public function declineRFP($rfp_id, $reason) {
            ...
            return response()->json(['success' => true, 'view_data' => 'Declined Successfully !']);
        }
        /*Get Team*/
        ...
        public function getTeamstore(Request $request) {
            $this->validate($request, ["team_name" => "required|max:191", "age_group" => "required|max:191", "gender" => "required", ]);
            /*For hotel type logo*/
            $team = new Team();
        ...
        public function uploadRoster($rfp_id, $team) {
            ...
            return response()->json(['success' => true, 'view_data' => 'Team is added to the trip']);
        }
        ...
        public function show_trips() {
            if (Session::get('level') != 4) return redirect(URL("/"));
            ...
            $data_submit = usertrips::where("status", 6)->where('entry_by', session('uid'))->get();
            return view('coordinator.viewtrips', compact('trips', 'amenities', 'data_client', 'purchases', 'data_submit', 'data_accept', 'client'));
        }
        public function show_trip_detail($id) {
            ...
            return view('coordinator.tripsingle', compact('trip', 'rfp', 'data2', 'trip_id_new', 'data_hotel', 'data', 'rfps_new'));
        }
    }
