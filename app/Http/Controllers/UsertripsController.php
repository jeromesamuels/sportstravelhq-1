...
//return response()->json(['status'=>'success','messagetext'=>"Thankyou! we've got your booking request. Our travel coordinator will contact you soon.!"]);
...
DB::table('rfps')->where('id', $rfp_id)->update(['status' => 2]);
	...
	if ($agree_id === null) {
	DB::insert("insert into agreement_forms(id,sender_id,reciever_id,reciever_group,coordinator_id,reciever_email,hotel_name,hotel_details,agreement_text,for_rfp) values ($rfp_id,$log_id,$aggree->user_id,$group->group_id,$log_id,'$group->email','$hotel->name','$hotel->address','$aggree->hotels_message',$rfp_id)");
	}
	else{
	return response()->json([
	'success' => true, 
...
public function uploadRostertore(Request $request){
	...
	function show_trips() 
	{
		...
		return view('coordinator.viewtrips', compact('trips', 'amenities'));
