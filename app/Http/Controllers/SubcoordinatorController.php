...
class SubcoordinatorController extends Controller {
	...
	public $module = 'subcoordinator';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Subcoordinator();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'subcoordinator',
	 ...
	 function postSave($request, $id = 0) {
            ...
            if ($validator->passes()) {
                ...
                if (!is_null($request->input('apply'))) {
                    ...
                    $return = 'subcoordinator?return=' . self::returnUrl();
                }
	...
	public static function display(  )
	{
		...
		$model  = new Subcoordinator();
		$info = $model::makeInfo('subcoordinator');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('subcoordinator.public.view',$data);			
			}			
		} 
		else {
			...
			return view('subcoordinator.public.index',$data);	
		}
