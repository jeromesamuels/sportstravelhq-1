...
$user_invitition=DB::table('tb_users')->where('id',$request->input('id'))->get();
foreach ($user_invitition as $user_invitition_id) {
}
$rfps=DB::table('rfps')->where('sales_manager',$request->input('email'))->get();
$sales_manager=$user_invitition_id->first_name.''.$user_invitition_id->last_name;
if(count($invitition)==1 && count($rfps)==1){
$hotels=DB::table('hotels')->where('id',$request->input('hotel_id'))->get();
...
{
$this->data = array(
'invitations' => \DB::table('invitations')->where('group_id', 5)->get(), 
