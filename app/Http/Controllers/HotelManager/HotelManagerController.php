...
class HotelManagerController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $purchases = DB::table('invoices')->where('invoices.hotel_name', '=', $hotel->hotel_id)->sum('invoices.amt_paid');
        $purchases_due = DB::table('invoices')->where('invoices.hotel_name', '=', $hotel->hotel_id)->sum('invoices.est_amt_due');
    ...
    public function makeInvoice(Request $request) {
        $this->validate($request, ['trip_id' => 'numeric|min:0', 'actualized_room_count' => 'numeric|min:0', 'room_rate' => 'numeric', 'amt_paid' => 'numeric', ]);
