...
class UserController extends Controller {
    ...
    public function postCreate(Request $request) {
        $rules = array('username' => 'required|unique:tb_users', 'firstname' => 'required|alpha_num|min:2', 'lastname' => 'required|alpha_num|min:2', 'email' => 'required|email|unique:tb_users', 'password' => 'required|confirmed', 'password_confirmation' => 'required');
        ...
        if ($validator->passes()) {
            ...
            $request->input('phone'), array('from' => $twilio_number, 'body' => 'Thank You for registering with SportsTravel HQ! Please check your inbox and click on the activation link below ' . $new_code));
            $authen->save();
            $data = array('firstname' => $request->input('firstname'), 'lastname' => $request->input('lastname'), 'email' => $request->input('email'), 'password' => $request->input('password'), 'code' => $code, 'subject' => "[ " . $this->config['cnf_appname'] . " ] REGISTRATION ");
    ...
    public function guestLoginStore(Request $request) {
        $rules = array('email' => 'required|email|unique:tb_users',);
        $validator = Validator::make($request->all(), $rules);
        $authen = new User;
        if ($validator->passes()) {
            $authen->email = trim($request->input('email'));
    ...
    public function TripDetails($id, $email) {
        ...
        if (count($guest_user) == 1) {
            return view('hotelmanager.tripSingleguest', compact('trip', 'rfp', 'email', 'trip_id_new', 'invoice', 'purchases', 'trip_booking', 'rfps_new'));
        } else {
            return redirect('user/login');
        }
    }
    ...
    public function postSignin(Request $request) {
        ...
        if ($validator->passes()) {
            ...
            if (\Auth::attempt(array('email' => $request->input('email'), 'password' => $request->input('password')), $remember) or \Auth::attempt(array('username' => $request->input('email'), 'password' => $request->input('password')), $remember)) {
                if (\Auth::check()) {
                    ...
                    if ($row->active == 'Inactive') {
                        ...
                        $session = array('gid' => $row->group_id, 'hid' => $row->hotel_id, 'uid' => $row->id, 'eid' => $row->email, 'phn' => $row->phone_number, 'll' => $row->last_login, 'fid' => $row->first_name . ' ' . $row->last_name, 'username' => $row->username, 'join' => $row->created_at, 'level' => $level);
                        ...
                        if ($request->ajax() == true) {
                            ...
                            if ($session['level'] == 2 || $session['level'] == 1 || $session['level'] == 4 || $session['level'] == 5 || $session['level'] == 6) {
                                if (!$request->has("send_code") && $row->vcode == 0) {
                                    ...
                                    $client->messages->create(
                                    // Where to send a text message (your cell phone?)
                                    $phone, array('from' => $twilio_number, 'body' => 'Your SportsTravelHQ verification code is ' . $code));
                ...
                if ($request->ajax() == true) {
                    ...
                    return redirect('user/login')->with(['status' => 'error', 'message' => 'Your username/password combination was incorrect'])->withInput();
                }
            }
        } else {
            if ($request->ajax() == true) {
                ...
                return redirect('user/login')->with(['status' => 'error', 'message' => 'The following  errors occurred'])->withErrors($validator)->withInput();
            }
        }
    }
    ...
    public function postSaveprofile(Request $request) {
        if (!\Auth::check()) return redirect('user/login');
        $rules = array('first_name' => 'required|alpha_num|min:2', 'last_name' => 'required|alpha_num|min:2',);
        ...
        if ($validator->passes()) {
            ...
            return redirect('user/profile')->with('message', 'The following errors occurred')->with('status', 'error')->withErrors($validator)->withInput();
        }
    }
    public function postSavepassword(Request $request) {
        $rules = array('password' => 'required', 'password_confirmation' => 'required');
        $validator = Validator::make($request->all(), $rules);
        if ($validator->passes()) {
            $user = User::find(\Session::get('uid'));
            ...
            return redirect('user/profile')->with(['status' => 'error', 'message' => 'The following errors occurred'])->withErrors($validator)->withInput();
        }
    }
    public function postSavehotel(Request $request) {
        if ($request->has('assign_hotel')) {
            \DB::table('tb_users')->where('id', $request->user_id)->update(['hotel_id' => $request->hotel_id]);
    ...
    public function postRequest(Request $request) {
        $rules = array('credit_email' => 'required|email');
        $validator = Validator::make($request->all(), $rules);
        if ($validator->passes()) {
            ...
            if ($user->count() >= 1) {
                ...
                $affectedRows = User::where('email', '=', $user->email)->update(array('reminder' => $request->input('_token')));
    ...
    public function postDoreset(Request $request, $token = '') {
        $rules = array('password' => 'required|alpha_num|confirmed', 'password_confirmation' => 'required|alpha_num');
        $validator = Validator::make($request->all(), $rules);
        if ($validator->passes()) {
            $user = User::where('reminder', '=', $token);
            if ($user->count() >= 1) {
    ...
    function autosocialize($social) {
        ...
        if (count($users)) {
            ...
            return redirect('user/login')->with(['message' => 'You have not registered yet ', 'status' => 'error']);
        }
    }
        ...
        if (is_null($id)) {
            ...
            if (\Auth::check()) {
                ...
                if ($row->active == '0') {
                    ...
                    $session = array('gid' => $row->group_id, 'hid' => $row->hotel_id, 'uid' => $row->id, 'eid' => $row->email, 'll' => $row->last_login, 'fid' => $row->first_name . ' ' . $row->last_name, 'username' => $row->username, 'join' => $row->created_at);
