...
class CorporateUsersController extends Controller {
    ...
    function create( Request $request ) 
    {
        ...
        if($this->access['is_add'] ==0) 
          ...
          $user_id = 5;
           $hotelid = DB::table('tb_users')->where('id', $user_id)->pluck('hotel_id');    
           
            foreach($hotelid as $item_usernm) {
                    $hotelid_new = $item_usernm;
                }
          
                   
               $hotel_type = DB::table('hotels')->where('id', $hotelid_new)->pluck('type');
    ...
    function postDoinvite( Request $request)
    {
        ...
        if ($validator->passes()) 
        {
            ...
            return redirect('corporate.user'.$request->input('redirect_to'))->with('message', 'The following errors occurred')->with('status','error')
            ->withErrors($validator)->withInput();
