        ...
        foreach ($trip_statuses as $trip_status) {
            ...
            if (count($guest_users) > 0) {
                foreach ($guest_users as $guest_users_new) {
                    ...
                    $data_guest       = array("trip_id"          => $trip_guest,
                                              "trip_name"        => $user_trip->trip_name,
                                              "adddress"         => $user_trip->from_address_1,
                                              "city"             => $user_trip->from_city,
                                              "state"            => $user_trip->from_state_id,
                                              "zip_code"         => $user_trip->from_zip,
                                              "check_in"         => $user_trip->check_in,
                                              "check_out"        => $user_trip->check_out,
                                              "budget_from"      => $user_trip->budget_from,
                                              "budget_to"        => $user_trip->budget_to,
                                              "double_queen_qty" => $user_trip->double_queen_qty,
                                              "double_king_qty"  => $user_trip->double_king_qty,
                                              "added"            => $user_trip->added,
                                              "coordinator_name" => $coordinator_name,
                                              'subject_guest'    => $subject_guest,
                                              "email"            => $to_guest,
                    );
            ...
            if ($trip_status->group_level == 10) {
                ...
                $user_trip = DB::table('user_trips')->where('id', '=', $rfp_id)->first();
            ...
            $trip_status_logs[] = array(
                ...
                'location'              => $location->regionName . ' <br> ' . $location->cityName . ' , ' . $location->zipCode,
            );
//dd($trip_status_logs);
        }
